    ...
    <div class="col-md-8 mx-auto">
        <div class="card shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center border-bottom mb-0">
                ...
                <small class="text-muted float-end">Update existing product information</small>
            </div>
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('items.update', $item) }}">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" class="form-control bg-light text-muted" id="code" value="{{ $item->code }}" readonly>
                            </div>
                            <small class="text-muted small">Code cannot be modified</small>
                        </div>
                    </div>
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <select class="form-select @error('uom') is-invalid @enderror" id="uom" name="uom" required>
                                    @foreach($uomOptions as $option)
                                        <option value="{{ $option }}" {{ (old('uom', $item->uom) == $option) ? 'selected' : '' }}>{{ $option }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('items.index') }}" class="btn btn-label-secondary">Cancel</a>
