...
@section('title', 'Create Purchase Delivery Challan')

@section('content')
<div class="card shadow-sm border-0 mb-4" style="border-radius: 12px;">
    <div class="card-header bg-white py-3 border-bottom">
        <div class="d-flex justify-content-between align-items-center">
            <h4 class="mb-0 text-primary fw-bold font-outfit"><i class="fas fa-plus-circle me-2"></i>Create Purchase Delivery Challan</h4>
            <span class="badge bg-label-primary px-3 py-2">New Entry</span>
        </div>
    </div>
    <div class="card-body p-4">
        <form id="pdcForm" method="POST" action="{{ route('purchase_delivery_challans.store') }}">
            @csrf
            ...
            <div class="row g-2 mb-4">
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control form-control-sm bg-light" id="challan_number" name="challan_number" value="{{ $challanNumber }}" readonly>
                </div>
                <div class="col-md-3">
                    <label for="supplier_dc_number" class="form-label mb-1">SUPPLIER DC#</label>
                    <input type="text" class="form-control form-control-sm" id="supplier_dc_number" name="supplier_dc_number" placeholder="Enter Reference #">
                </div>
                <div class="col-md-3">
                    <label for="date" class="form-label mb-1">DATE</label>
                    <input type="date" class="form-control form-control-sm" id="date" name="date" required value="{{ date('Y-m-d') }}">
                </div>
                <div class="col-md-3">
            ...
            <div class="row g-2 mb-4">
                <div class="col-md-2">
                    ...
                    <input type="text" class="form-control form-control-sm" id="vehicle_number" name="vehicle_number" placeholder="LHR-1234">
                </div>
                <div class="col-md-4">
                    <label for="supplier_address" class="form-label mb-1">SUPPLIER ADDRESS</label>
                    <textarea class="form-control form-control-sm bg-light" id="supplier_address" rows="1" readonly placeholder="Auto-fetched address"></textarea>
                </div>
                <div class="col-md-6">
                    <label for="remarks" class="form-label mb-1">REMARKS</label>
                    <textarea class="form-control form-control-sm" id="remarks" name="remarks" rows="1" placeholder="Optional notes..."></textarea>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered align-middle table-sm custom-pdc-table" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            <tr class="item-row">
                                <td class="text-center sn-col small fw-bold text-muted">1</td>
                                <td>
                                    <select class="form-select form-select-sm select2 item-select" name="items[0][purchase_item_id]" required>
                                        <option value="">Select Item</option>
                                    </select>
                                </td>
                                <td>
                                    <input type="number" step="0.01" class="form-control form-control-sm text-center qty-input" name="items[0][quantity]" required>
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm text-center bg-light unit-input" name="items[0][unit]" readonly>
                                </td>
                                <td>
                                    <input type="number" step="0.01" class="form-control form-control-sm text-end rate-input" name="items[0][rate]" required>
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm text-end bg-light amount-input" readonly value="0.00">
                                </td>
                                <td>
                                    <input type="number" step="0.01" class="form-control form-control-sm text-center tax-percent-input" name="items[0][tax_percent]" value="0">
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm text-end bg-light tax-amount-input" readonly value="0.00">
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm text-end bg-light total-amount-input fw-bold" readonly value="0.00" style="color: #0d6efd;">
                                </td>
                                <td class="text-center">
                                    <button type="button" class="btn btn-link text-danger p-0 remove-row" disabled title="Delete Row"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot class="bg-light fw-bold border-top-2">
                            <tr>
                                <td colspan="10" class="text-end py-2 px-4">
                                    ...
                                    <span class="text-primary fs-5" id="grand-total">0.00</span>
                                </td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <div class="mt-5 border-top pt-4 text-end">
                ...
                <button type="submit" class="btn btn-primary px-5 fw-bold shadow-sm btn-sm"><i class="fas fa-save me-2"></i>SAVE PURCHASE DC</button>
            </div>
        </form>
    </div>
</div>
...
<style>
    /* COLUMN WIDTH FLEXIBILITY - Tweak these variables to adjust the table layout */
    :root {
        --col-sn-width: 40px;
        --col-item-width: auto; /* Auto makes it fill remaining space */
        --col-qty-width: 75px;
        --col-uom-width: 60px;
        --col-rate-width: 95px;
        --col-amt-width: 110px;
    ...
    .custom-pdc-table {
        table-layout: fixed; /* Ensures fixed widths are honored */
        width: 100%;
    }
    .custom-pdc-table input, 
    .custom-pdc-table select, 
    .custom-pdc-table .select2-container--bootstrap-5 .select2-selection {
    ...
    .custom-pdc-table .select2-container--bootstrap-5 .select2-selection--single {
        height: 28px !important;
    }
    .custom-pdc-table .select2-container--bootstrap-5 .select2-selection__rendered {
